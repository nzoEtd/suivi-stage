...
use App\Models\EtudiantTdAnneeUniv;
...
class EtudiantTdAnneeUnivController extends Controller
{
    ...
    {
        try {
            $relations = EtudiantTdAnneeUniv::all();
    ...
    public function store(Request $request)
    {
        try {
            $donneesValidees = $request->validate([
                ...
                'idTD' => 'bail|required|integer|exists:t_d_s,idTD',
            ...
            $relation = EtudiantTdAnneeUniv::create($donneesValidees);
    ...
    {
        try {
            $query = EtudiantTdAnneeUniv::query();

            if ($request->filled('idUPPA')) {
                $query->where('idUPPA', $request->idUPPA);
            }

            if ($request->filled('idTD')) {
                $query->where('idTD', $request->idTD);
            }
    ...
    public function destroy(Request $request)
    {
        try {
            $donneesValidees = $request->validate([
                ...
                'idTD' => 'bail|required|integer',
                'idAnneeUniversitaire' => 'bail|required|integer',
            ]);

            $deleted = EtudiantTdAnneeUniv::where($donneesValidees)->delete();
