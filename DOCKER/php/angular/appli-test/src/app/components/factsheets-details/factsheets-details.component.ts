...
@Component({
  ...
  imports: [
    ...
    LoadingComponent,
    BreadcrumbComponent,
    TutorAttributionModalComponent,
  ],
...
export class SheetDetailsComponent implements OnInit {
      ...
      switchMap((sheet) => {
        ...
        for (const [key, value] of Object.entries(sheet)) {
          ...
          if (typeof value === "object" && value !== null) {
            transformedSheet[newKey] =
              "value" in value ? (value as { value: any }).value : value;
          } else {
        ...
        if (this.detailsSheet?.idEntreprise) {
          observables["company"] = this.companyService.getCompanyById(
            this.detailsSheet.idEntreprise,
            [
              "idEntreprise",
              "raisonSociale",
              "adresse",
        ...
        if (this.detailsSheet?.idTuteur) {
          observables["companyTutor"] =
            this.companyTutorService.getCompanyTutorById(
              this.detailsSheet.idTuteur,
            );
        }
        ...
        if (this.detailsSheet?.idUPPA && this.currentAcademicYear) {
          return forkJoin(observables).pipe(
            switchMap((baseResults) => {
              ...
              if (baseResults["companyTutor"])
                this.companyTutor = baseResults["companyTutor"];

              // Charger le tuteur enseignant
              if (
                !this.detailsSheet?.idUPPA ||
                !this.currentAcademicYear?.idAnneeUniversitaire
              ) {
                return of(null);
              }
              return this.studentStaffAcademicYearService
                .getTutorByUppaAndYear(
                  this.detailsSheet.idUPPA.toString(),
                  this.currentAcademicYear.idAnneeUniversitaire,
                )
                .pipe(
                  switchMap((affectation) => {
                    if (affectation && affectation.idPersonnel) {
                      return this.staffService
                        .getStaffById(affectation.idPersonnel)
                        .pipe(
                          map((staff) => {
                            if (staff) {
  ...
  goToEdit() {
    if (this.detailsSheet?.idFicheDescriptive) {
      this.navigationService.navigateToDescriptiveSheetEditForm(
        this.detailsSheet.idFicheDescriptive,
      );
    }
  }
    ...
    if (this.detailsSheet?.idUPPA && this.detailsSheet?.idFicheDescriptive) {
      ...
      this.studentStaffAcademicYearService
        ...
        .subscribe({
          next: (rawData) => {
            if (rawData) {
              let tutorList: algorithmResponse[] = Object.entries(rawData).map(
                ([id, data]: [string, any]) => ({
                  ...
                  etudiantPresentEntreprise:
                    !!data.ETUDIANT_DEJA_PRESENT_ENREPRISE,
  ...
  handleConfirmAttribution(teacherId: number) {
    ...
    const selectedTeacher = this.teachers?.find(
      (teacher) => teacher.idPersonnel === teacherId,
    );
      ...
      .pipe(
        switchMap((existing) => {
          const operation = existing
            ? this.studentStaffAcademicYearService.updateStudentTeacherAssignments(
                affectation,
              )
            : this.studentStaffAcademicYearService.addStudentTeacherAssignments(
                affectation,
              );
          return operation;
        }),
