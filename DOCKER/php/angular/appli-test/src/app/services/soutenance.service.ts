...
export class SoutenanceService {
  ...
  getSoutenanceById(id: number, fields?: string[]): Observable<Soutenance | undefined> {
    let params = new HttpParams();
    ...
    return this.http.get<Soutenance>(`${this.apiUrl}/api/soutenance/${id}`, {params}).pipe(
      tap(response => this.log(response)),
      catchError(error => this.handleError(error, undefined))
    );
  }
  ...
  addSoutenance(soutenance: Soutenance): Observable<Soutenance> {
    ...
    return this.http.post<Soutenance>(`${this.apiUrl}/api/soutenance/create`, soutenance, httpOptions).pipe(
      tap(response => this.log(response)),
